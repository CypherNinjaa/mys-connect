/**
 * Socket event contract — the single source of truth shared by server, admin and mobile.
 *
 * Naming convention: `<domain>:<past-tense-action>`
 *   domain  — notice | event | gallery | member | dashboard | registration | connection
 *   action  — created | updated | deleted | published | unpublished | cancelled | approved
 *
 * Payloads are deliberately small. A socket event says *what changed* and carries
 * just enough data for a client to patch its cache in place. Clients must never
 * need a follow-up REST call to make sense of an event, and the server must never
 * push a payload big enough to make the socket a second, slower API.
 */
...
export interface NotificationPayload extends SocketEnvelope {
   ...
   *
   * Broadcasts insert with `createMany`, which returns a count rather than rows,
   * so a fan-out notification has no id to quote. Clients key off `data` instead
   * (which carries the entity type and id) and refetch the list when they need
   * the persisted row.
   */
  id?: string;
  title: string;
  ...
  unreadCount?: number;
  /** Set when the same change also went out over FCM, so clients can suppress a
   *  duplicate in-app banner for a push they are about to receive. */
  alsoPushed: boolean;
}
...
/**
 * Server → client event map. Socket.IO uses this to type `io.to(...).emit()`
 * and the clients use the mirrored copy to type their listeners.
 */
